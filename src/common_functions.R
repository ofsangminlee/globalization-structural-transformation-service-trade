...
plot.time.options <- function(x.limits = c(1995, 2018), bw = TRUE, ...){
    res <- list(xlab("Year"),
                ...
                theme(legend.position = "none", axis.text = element_text(size = 13), text = element_text(size = 15), panel.grid.minor.x = element_blank())# If I don't set sizes for  axis.text separately, due to the incompleteness of the theme, 15*0.8 (default rel size) will be the default size for axis.text.
                )
    if (bw){
...
default.color.base  <- c("CS" = "blue", "PS" = "red", "G" = "black")
default.color.alt <- c("BTS" = "green4", "HTS" = "orange", "G" = "black")
default.color.gs <- c("S" = "purple", "G" = "black")

## Default shapes for plotting
default.shape.base <- c("G" = 17, "PS" = 15, "CS" = 16)
default.shape.alt <- c("G" = 17, "HTS" = 15, "BTS" = 16)
default.shape.gs <- c("G" = 17, "S" = 15)

## Default linetypes for plotting
default.linetype.base <- c("G" = "solid", "PS" = "dashed", "CS" = "dotted")
default.linetype.alt <- c("G" = "solid", "HTS" = "dashed", "BTS" = "dotted")
