...
nx.gdp.usa$ind <- ifelse(nx.gdp.usa$ind == "g", "Goods",
                  ifelse(nx.gdp.usa$ind == "cs", "Consumer services", "Producer services"))
nx.gdp.usa$ind <- factor(nx.gdp.usa$ind, levels = c("Goods", "Producer services", "Consumer services"))
...
ggplot(nx.gdp.usa, aes(x = year, y = nx, by = ind)) +
    ...
    scale_linetype_manual(values = c("Goods" = "solid", "Producer services" = "dashed", "Consumer services" = "dotted")) +
    scale_shape_manual(values = c("Goods" = 17, "Producer services" = 15, "Consumer services" = 16)) +
...
epss <- c("g" = 1, "cs" = params.pref$estimates[params.pref$params == "eps.cs"], "ps" = params.pref$estimates[params.pref$params == "eps.ps"])
...
recover.phi <- function(ex.country, i.year, method = "year"){
    ...
    phi.cs <- res.phi$par[1]
    phi.ps <- res.phi$par[2]
