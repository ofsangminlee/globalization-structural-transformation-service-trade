...
clean.tau <- function(dat.tau, method = "wedge"){
    ...
    tau.2$origin.destination <- ifelse(tau.2$origin.hl == 0 & tau.2$destination.hl == 0, "DEV-DEV",
                                ifelse(tau.2$origin.hl == 0 & tau.2$destination.hl == 1, "DEV-ADV",
                                ifelse(tau.2$origin.hl == 1 & tau.2$destination.hl == 0, "ADV-DEV", "ADV-ADV")))
...
prepare.plot <- function(tau, method = "wedge", is.diff = FALSE){
    ...
    tau.plot <- subset(tau.plot, ind %in% c("G", "PS"))
...
summary.tau <- function(trade.mat, tau.mat, list.from, list.to, method = "wedge"){
    ...
    ## Trade-weighted mean
    res.wm <- sum(tau.mat[tau.mat$country %in% list.from, list.to]*trade.mat[trade.mat$country %in% list.from, list.to])/sum(trade.mat[trade.mat$country %in% list.from, list.to])
...
summary.22 <- function(i.ind, i.year, dat.trade, dat.tau){
    ...
    res.aa <- add.names(res.aa, "ADV", "ADV", i.ind, num.year)
    
    res.ad <- summary.tau(trade.mat, tau.mat, list.adv, list.dev)
    res.ad <- add.names(res.ad, "ADV", "DEV", i.ind, num.year)
    
    res.da <- summary.tau(trade.mat, tau.mat, list.dev, list.adv)
    res.da <- add.names(res.da, "DEV", "ADV", i.ind, num.year)
    
    res.dd <- summary.tau(trade.mat, tau.mat, list.dev, list.dev)
    res.dd <- add.names(res.dd, "DEV", "DEV", i.ind, num.year)
...
summary.plot <- subset(summary.year, origin.destination != "ALL-ALL" & ind != "cs")
summary.plot$ind <- toupper(summary.plot$ind)
summary.plot <- add.title(summary.plot)

summary.plot.hs <- subset(summary.year.hs, origin.destination != "ALL-ALL" & ind != "cs")
...
plot.median.22(summary.plot) + plot.time.options(x.limits = c(1990, 2018), bw = TRUE) + facet_wrap(~title.alp)
dev.off()

pdf(file = "../../doc/figures/trade_cost_22_beamer.pdf", width = 10, height = 6)
plot.median.22(summary.plot) + plot.time.options(x.limits = c(1990, 2018), bw = FALSE) + facet_wrap(~title)
dev.off()

pdf(file = "../../doc/figures/trade_cost_22_paper_hs.pdf", width = 10, height = 6)
plot.median.22(summary.plot.hs) + plot.time.options(x.limits = c(1990, 2018), bw = TRUE) + facet_wrap(~title.alp)
dev.off()

pdf(file = "../../doc/figures/trade_cost_22_beamer_hs.pdf", width = 10, height = 6)
plot.median.22(summary.plot.hs) + plot.time.options(x.limits = c(1990, 2018), bw = FALSE) + facet_wrap(~title)
dev.off()
...
plot.ex.im <- function(dat.plot.tau, ex.country, list.inds, default.color.base, default.linetype.base, default.shape.base, plot.title, bw2 = TRUE)
    ggplot(subset(dat.plot.tau, country == ex.country & ind %in% list.inds & type %in% c("Export cost", "Import cost")), aes(x = year, y = value)) +
        ...
        ylab("Trade costs") +
        ggtitle(plot.title) +
        plot.time.options(x.limits = c(1985, 2018), bw = bw2)
...
plot.rel <- function(dat.plot.tau, ex.country, list.inds, default.color.base, default.linetype.base, default.shape.base, plot.title, bw2 = TRUE)
    ggplot(subset(dat.plot.tau, country == ex.country & ind %in% list.inds & type %in% c("Relative export cost")), aes(x = year, y = log(value))) +
        ...
        ggtitle(plot.title) +
        plot.time.options(x.limits = c(1994, 2018), bw = bw2)
...
plot.one.paper <- function(ex.country, adjust.grid.b = FALSE, y.limits.1.b = c(-10,10), y.limits.2.b = c(-10,10))
    plot.one.country(dat.plot.tau.weighted, ex.country, c("G", "PS"), default.color.base, default.linetype.base, default.shape.base, "(i) Trade costs (weighted means)", "(ii) Log(export cost/import cost)", adjust.grid = adjust.grid.b, y.limits.1 = y.limits.1.b, y.limits.2 = y.limits.2.b)

plot.one.beamer <- function(ex.country, adjust.grid.b = FALSE, y.limits.1.b = c(-10,10), y.limits.2.b = c(-10,10))
    plot.one.country(dat.plot.tau.weighted, ex.country, c("G", "PS"), default.color.base, default.linetype.base, default.shape.base, "Trade costs (weighted means)", "(ii) Log(export cost/import cost)", bw3 = FALSE, adjust.grid = adjust.grid.b, y.limits.1 = y.limits.1.b, y.limits.2 = y.limits.2.b)

plot.one.paper.hs <- function(ex.country, adjust.grid.b = FALSE, y.limits.1.b = c(-10,10), y.limits.2.b = c(-10,10))
    plot.one.country(dat.plot.tau.weighted.hs, ex.country, c("G", "PS"), default.color.base, default.linetype.base, default.shape.base, "(i) Trade costs (weighted means)", "(ii) Log(export cost/import cost)", adjust.grid = adjust.grid.b, y.limits.1 = y.limits.1.b, y.limits.2 = y.limits.2.b)

plot.one.beamer.hs <- function(ex.country, adjust.grid.b = FALSE, y.limits.1.b = c(-10,10), y.limits.2.b = c(-10,10))
    plot.one.country(dat.plot.tau.weighted.hs, ex.country, c("G", "PS"), default.color.base, default.linetype.base, default.shape.base, "Trade costs (weighted means)", "(ii) Log(export cost/import cost)", bw3 = FALSE, adjust.grid = adjust.grid.b, y.limits.1 = y.limits.1.b, y.limits.2 = y.limits.2.b)

## Plot (China)
pdf(file = "../../doc/figures/tau_country_chn_paper.pdf", width = 10, height = 6)
...
pdf(file = "../../doc/figures/tau_country_chn_paper_hs.pdf", width = 10, height = 6)
...
tau.index <- construct.index(tau.country.weighted, c("g", "ps"))

tau.index.hs <- construct.index(tau.country.weighted.hs, c("g", "ps")) # NOTE: even in the H-S setup, the index changes, because of the weights used for exports and imports are different.

tau.country.weighted.alt <- calc.w.ex.im(icio.trade.ind.alt, tau.alt, list.reclass.alt)
tau.index.alt <- construct.index(tau.country.weighted.alt, c("g", "hts"))
...
tau.index.low <- construct.index(tau.country.weighted.low, c("g", "ps"))

tau.country.weighted.high <- calc.w.ex.im(icio.trade.ind, tau.high, list.reclass)
tau.index.high <- construct.index(tau.country.weighted.high, c("g", "ps"))

tau.country.weighted.alt <- calc.w.ex.im(icio.trade.ind.alt, tau.alt, list.reclass.alt)
tau.index.alt <- construct.index(tau.country.weighted.alt, c("g", "hts"))

tau.country.weighted.orig <- calc.w.ex.im(icio.trade.ind, tau.orig, list.reclass)
tau.index.orig <- construct.index(tau.country.weighted.orig, c("g", "ps"))   
...
plot.ind <- plot.ind[, c("country", "glre.g", "glre.ps", "index", "gdppc")]
colnames(plot.ind)[4] <- "glre.index"
plot.ind <- pivot_longer(plot.ind, cols = c("glre.g", "glre.ps", "glre.index"), names_to = "ind")
plot.ind$ind <- toupper(substr(plot.ind$ind, 6, nchar(plot.ind$ind)))
plot.ind <- add.title(plot.ind)

pp <- ggplot(data = subset(plot.ind, ind %in% c("G", "PS")), aes(x = log(gdppc), y = value)) +
