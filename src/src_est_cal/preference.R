...
## Result
res.exp <- gmm(g.exp, data.matrix(cons.est), t0 = c(log.sigma = -0.5, log.eps.cs = 0.5, log.eps.ps = 0.5), gradv = Dg.exp, method = "BFGS")
...
res <- format.result(res.exp, c("sigma", "eps.cs", "eps.ps"))
res
...
res <- bootstrap.stderr(cons.est, g.exp, Dg.exp, res.exp, c("sigma", "eps.cs", "eps.ps"), 1000)
...
res.exp.alt <- gmm(g.exp.alt, data.matrix(cons.est.alt), t0 = c(log.sigma = -0.5, log.eps.bts = 0.5, log.eps.hts = 0.5), gradv = Dg.exp.alt, method = "BFGS")

res.alt <- format.result(res.exp.alt, c("sigma", "eps.bts", "eps.hts"))
res.alt

res.alt <- bootstrap.stderr(cons.est.alt, g.exp.alt, Dg.exp.alt, res.exp.alt, c("sigma", "eps.bts", "eps.hts"), 1000)
...
res.usa <- bootstrap.stderr(subset(cons.est, country != "USA"), g.exp, Dg.exp, res.exp.usa, c("sigma", "eps.cs", "eps.ps"), 1000)
