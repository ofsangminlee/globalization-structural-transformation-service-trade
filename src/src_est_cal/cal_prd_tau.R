...
lll <- unlist(check.below1(tau)) # G: 0.3, CS: 0.9 PS: 1.0 percent

writeLines(paste("Percentage of bilateral trade costs below one for G, CS, and PS is", lll["g"], ",", lll["cs"], ", and", lll["ps"]), "../../doc/nums/tau_below_one.txt")

## This one is to check triangular inequality. About 9 percent is broken down.
qwer <- as.matrix(tau[["g"]][[24]][, list.country.ordered])
...
writeLines(paste("Percentage of country-pairs for goods in 2018 with broken-down triangular equations is", sum(asdf < qwer)/(67*67)*100, "."), "../../doc/nums/triangle_2018_g.txt")
