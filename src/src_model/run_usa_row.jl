...
S = 3 # Number of sectors (column) in the order of G, CS, PS
...
σ = get_param("sigma")[1]
ϵ = [get_param("eps.g")[1], get_param("eps.cs")[1], get_param("eps.ps")[1]]
...
ϕ_zzz = Array(phis[phis.year .== "X1995", [:g, :cs, :ps]])
...
names_industries = string.("alpha.", ["g", "cs", "ps", "v"])
...
α_zzz[:,:,2] .= alphas_zzz[alphas_zzz.ind .== "cs", names_industries]
α_zzz[:,:,3] .= alphas_zzz[alphas_zzz.ind .== "ps", names_industries]
...
for i in 1:24 # 1995 to 2018
    ...
    t_temp[:,:,2] .= tau["cs"][i][:, list_countries]
    t_temp[:,:,3] .= tau["ps"][i][:, list_countries]
...
function get_A(product, var_name)
    ...
    for year in 1995:1995
        ...
        A_temp[:,2] .= product_temp[product_temp.ind .== "cs", var_name]
        A_temp[:,3] .= product_temp[product_temp.ind .== "ps", var_name]
        A_res = [A_res; [A_temp]]
    end
    return(A_res)
end
...
qwer./sum(qwer, dims = 2) # In the data, USA first-year GDP share is (22, 42, 36%) for (G, CS, PS)
...
function result(dat)
    ...
    rename!(temp, string.("va.", ["g", "cs", "ps"]))

    temp2 = DataFrame(vcat(dat.exp_share...), :auto)
    rename!(temp2, string.("exp.", ["g", "cs", "ps"]))
