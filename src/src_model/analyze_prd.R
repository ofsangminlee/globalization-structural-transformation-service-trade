...
clean.a <- function(a){
    ...
    a$ps <- a$ps/median(a$ps[a$year == 1995])
    a$cs <- a$cs/median(a$cs[a$year == 1995])
    
    a  <- a %>%
        group_by(year) %>%
        reframe(G = quantile(g, c(0.25, 0.5, 0.75)), CS = quantile(cs, c(0.25, 0.5, 0.75)), PS = quantile(ps, c(0.25, 0.5, 0.75)), type = c("Q1", "Q2", "Q3"))
    
    a <- pivot_longer(a, cols = c("G", "CS", "PS"), names_to = "ind")
...
clean.ga <- function(g.a){
    ...
    g.a <- pivot_longer(g.a, cols = c("G", "CS", "PS"), names_to = "ind")
...
g.la <- la
g.la <- pivot_longer(g.la, cols = c("g", "cs", "ps"), names_to = "ind", values_to = "prd")
...
gopl$relp.ps <- gopl$p.ps/gopl$p.g
gopl$relp.cs <- gopl$p.cs/gopl$p.g
relp <- gopl[, c("country", "year", "relp.ps", "relp.cs")]

sum.relp  <- relp %>%
    group_by(year) %>%
    reframe(relp.ps = quantile(relp.ps, c(0.25, 0.5, 0.75)), relp.cs = quantile(relp.cs, c(0.25, 0.5, 0.75)), type = c("Q1", "Q2", "Q3"))

relp.2 <- pivot_longer(relp, cols = c("relp.ps", "relp.cs"))
relp.2$ind <- relp.2$name
relp.2$name <- NULL
relp.2$ind <- substr(relp.2$ind, 6,7)
...
sum.g.relp <- g.relp %>%
    group_by(year) %>%
    reframe(PS = quantile(ps, c(0.25, 0.5, 0.75)), CS = quantile(cs, c(0.25, 0.5, 0.75)), type = c("Q1", "Q2", "Q3"))

sum.g.relp <- pivot_longer(sum.g.relp, cols = c("PS", "CS"), names_to = "ind", values_to = "value")

two.sec.title <- data.frame(ind = c("PS", "CS"), title = c("Producer Services", "Consumer Services"), title.rom = c("(i) Producer Services", "(ii) Consumer Services"))

sum.g.relp <- merge(sum.g.relp, two.sec.title, by = "ind", all.x = TRUE)

sum.g.relp$title <- factor(sum.g.relp$title, levels = c("Producer Services", "Consumer Services"))
...
plot.relp(sum.g.relp, y.lab = "Quartiles (growth factor of price relative to goods)", type.title = "title.rom")
dev.off()

pdf(file = "../../doc/figures/price_gr_trends_beamer.pdf", width = 10, height = 6)
plot.relp(sum.g.relp, y.lab = "Quartiles (growth factor of price relative to goods)", bw.2 = FALSE)
dev.off()
...
sum(g.rela.18$cs > g.rela.18$g)
sum(g.rela.18$ps > g.rela.18$g)

writeLines(paste("The number of countries, where prices of CS and PS relative to G fell is", sum(g.relp.18$cs < 1), "and", sum(g.relp.18$ps < 1), ".",
                 "The number of countries, where labor productivity was faster in CS and PS compared to G, is", sum(g.rela.18$cs > g.rela.18$g), "and", sum(g.rela.18$ps > g.rela.18$g)), "../../doc/nums/productivity_counts.txt")
