...
## 2018 result & divide S into PS and CS
...
motivation.num.total.trade <- int.tr.2018[, c("export", "g.cs.ps")] %>%
    group_by(g.cs.ps) %>%
...
default.color  <- c("CS" = "blue", "PS" = "red", "G" = "black")

plot.tradedness.intermediateness <- function(dat, size.repel = 5, my.theme = NULL, var.color = "g.cs.ps", pal.color = default.color)
    ggplot(dat, aes(x = tradedness, y = intermediateness)) + geom_point(aes(color = !!sym(var.color), shape = !!sym(var.color)), size = 3) +
        geom_text_repel(aes(label = short.name, color = !!sym(var.color)), nudge_y = 0.00, size = 5) +
        scale_color_manual(values = pal.color) +
        labs(x = "Total exports / total gross output", y = "Total intermediate usage / total gross output") +
        my.theme +
...
## Save three-sector industry classification.
...
int.tr.2018.g$g.cs.ps <- "CS"
int.tr.2018.g$g.cs.ps[int.tr.2018.g$intermediateness > 0.4] <- "PS"
int.tr.2018.g$g.cs.ps[int.tr.2018.g$country.ind %in% list.ind$code[list.ind$ISIC4_one %in% LETTERS[1:5]]] <- "G"
...
int.tr.2018.g <- rbind(int.tr.2018.g, subset(int.tr.2018, g.cs.ps == "G"))
...
## Instead of PS and CS, I can do highly tradable vs barely tradable services by reclassifying the two industries.

## Check 1. how much does the two sector account for in gross output and trade?
dat.aa <- int.tr.2018[, c("country.ind", "short.name", "g.cs.ps", "export", "total")]
dat.aa <- subset(dat.aa, g.cs.ps!= "G")
dat.aa <- data.frame(ind = c("CS", "PS", "AA"),
                      go = c(sum(dat.aa$total[dat.aa$g.cs.ps == "CS"]), sum(dat.aa$total[dat.aa$g.cs.ps == "PS"]), sum(dat.aa$total[dat.aa$short.name %in% c("Art, entertainment, recreation", "Accommodation & food")])),
                      export = c(sum(dat.aa$export[dat.aa$g.cs.ps == "CS"]), sum(dat.aa$export[dat.aa$g.cs.ps == "PS"]), sum(dat.aa$export[dat.aa$short.name %in% c("Art, entertainment, recreation", "Accommodation & food")]))
                     )
write.csv(dat.aa, "../../doc/nums/fraction_aa.csv", row.names = FALSE)

## About 11% of CS and PS
dat.aa$go[dat.aa$ind == "AA"]/dat.aa$go[dat.aa$ind == "CS"]
dat.aa$go[dat.aa$ind == "AA"]/dat.aa$go[dat.aa$ind == "PS"]
dat.aa$go[dat.aa$ind == "AA"]/(dat.aa$go[dat.aa$ind == "PS"] + dat.aa$go[dat.aa$ind == "CS"])

## About 7% of total trade
dat.aa$export[dat.aa$ind == "AA"]/(dat.aa$export[dat.aa$ind == "CS"] + dat.aa$export[dat.aa$ind == "PS"])
dat.aa$export[dat.aa$ind == "AA"]/dat.aa$export[dat.aa$ind == "PS"]
dat.aa$export[dat.aa$ind == "AA"]/dat.aa$export[dat.aa$ind == "CS"]
...
alt.color <- c("BTS" = "green4", "HTS" = "orange", "G" = "black")

pdf(file = "../../doc/figures/int_tr_2018_alt.pdf", width = 10, height = 6)
plot.tradedness.intermediateness(int.tr.2018, var.color = "g.bts.hts", pal.color = alt.color)
dev.off()

pdf(file = "../../doc/figures/int_tr_2018_alt_paper.pdf", width = 10, height = 6)
plot.tradedness.intermediateness(int.tr.2018, var.color = "g.bts.hts", pal.color = alt.color, my.theme = theme_bw())
dev.off()

## Another three-sector industry classification.
list.ind.three.alt <- read.csv("../../output/cleaned_data/list_industry_two.csv")
ind.bts <- int.tr.2018$country.ind[int.tr.2018$tradedness < 0.05]
ind.hts <- setdiff(int.tr.2018$country.ind, c(ind.bts, "g"))
list.ind.three.alt$g.s <- list.ind.three.alt$gs
list.ind.three.alt$gs <- NULL
list.ind.three.alt$g.bts.hts <- ifelse(list.ind.three.alt$g.s == "g", "g",
                          ifelse(list.ind.three.alt$code %in% ind.bts, "bts",
                          ifelse(list.ind.three.alt$code %in% ind.hts, "hts", NA)))
write.csv(list.ind.three.alt, file = "../../output/cleaned_data/list_industry_three_alt.csv", row.names = FALSE)
...
tr.type.2018 <- merge(tr.type.2018, int.tr.2018[, c("country.ind", "g.cs.ps", "short.name")], by.x = "ind", by.y = "country.ind")

tr.type.sum <- tr.type.2018 %>%
    group_by(g.cs.ps) %>%
    summarize(c.no.export = sum(c.no.export), int.export = sum(int.export), dpabr.export = sum(DPABR.export))

tr.type.sum$total.export <- rowSums(tr.type.sum[, colnames(tr.type.sum) != "g.cs.ps"])
