...
clean.ind.name <- function(dat.cons){
    ...
    dat.cons$title <- factor(dat.cons$title, levels = c("Goods", "Producer Services", "Consumer Services"))

    dat.cons$title.alp <- factor(dat.cons$title.alp, levels = c("(a) Goods", "(b) Producer Services", "(c) Consumer Services"))
...
str.plot(dat.cons, "Share of final expenditure", default.color.base, default.shape.base) + facet_wrap(~title.rom)
dev.off()
...
str.plot.name(dat.cons.name, "Share of final expenditure", default.color.base, default.shape.base) + facet_wrap(~title.rom)
dev.off()
...
above.zero(dat.cons.diff, "PS")
above.zero(dat.cons.diff, "CS")

all.correct <- function(dat.cons.diff)
    sum(dat.cons.diff$lg.share[dat.cons.diff$ind == "G"] < 0 &
        dat.cons.diff$lg.share[dat.cons.diff$ind == "PS"] > 0 &
        dat.cons.diff$lg.share[dat.cons.diff$ind == "CS"] > 0)
...
str.plot(dat.prod, "Share of GDP", default.color.base, default.shape.base) + facet_wrap(~title.rom)
dev.off()
...
str.plot.name(dat.prod.name, "Share of GDP", default.color.base, default.shape.base) + facet_wrap(~title.rom)
dev.off()
...
str.plot.diff(dat.prod.diff, "Change in the GDP shares from 1995 to 2018", default.color.base, default.shape.base) + facet_wrap(~title.rom)
dev.off()

above.zero(dat.prod.diff, "G")
above.zero(dat.prod.diff, "PS")
above.zero(dat.prod.diff, "CS")

all.correct(dat.prod.diff)

writeLines(paste("For consumption, numbers of exceptions are", above.zero(dat.cons.diff, "G"), "for G,", 66 - above.zero(dat.cons.diff, "PS"), "for PS,", 66 - above.zero(dat.cons.diff, "CS"), "for CS."), "../../doc/nums/str_chg_expenditure_change_exception.txt")

writeLines(paste("For production, numbers of exceptions are", above.zero(dat.prod.diff, "G"), "for G,", 66 - above.zero(dat.prod.diff, "PS"), "for PS,", 66 - above.zero(dat.prod.diff, "CS"), "for CS."), "../../doc/nums/str_chg_prd_change_exception.txt")
...
writeLines(paste("Services share of world total GDP is", sum(dat.18$tot.va[dat.18$ind %in% c("ps", "cs")])/sum(dat.18$tot.va)), "../../doc/nums/service_share_gpd.txt")
