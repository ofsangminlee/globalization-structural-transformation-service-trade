...
## 1. Calculate total input usage (volume) for aggregate sectors G, CS, and PS. E.g. for G, how much KLEMS did it employ?
...
## 2.1. Input weights
## Input usage (volume) for g, cs, ps
...
for (i in c(q.gap.agg, weight.input.agg.raw)){
    stopifnot(identical(i$ind, c("cs", "g", "ps")))
}
