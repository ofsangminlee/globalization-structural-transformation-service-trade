...
## A. Aggregate local currency price deflators into three reclassified industries (g, cs, ps), alternative three industry classification (g, bts, hts), two industry classification (g, s)
...
list.re <- c("total", "g", "cs", "ps")
list.re.alt <- c("total", "g", "bts", "hts")
...
## For G-CS-PS, 19 countries with full years, 4 countries with missing 1 or 2 years. For G-BTS-HTS, 16 + 2)
...
list.cs <- list.ind.three$code[list.ind.three$g.cs.ps == "cs"]
list.ps <- list.ind.three$code[list.ind.three$g.cs.ps == "ps"]

list.g <- c("D01T03", "D05T39") ## Agriculture, hunting, forestry and fishing [A], Industry including energy [B-E]

list.cs <- paste0("D", list.cs)

list.ps.disagg <- paste0("D", list.ps)
list.ps <- c("D45T47", "D49T53", "D58T63", "D64T66", "D69T75", "D77T82")  # Change industries into 1 digit ISIC for PS. (Some countries report the industries of PS in a less aggregated ways.)

list.ind <- c(list.g, list.cs, list.ps)

## Robustness: alternative three industries (G-BTS-HTS) vs two industry (G-S)
ind.aa <- c("D55T56", "D90T93") # Accommodation / Arts
list.bts <- setdiff(list.cs, ind.aa)
setequal(list.bts, paste0("D", list.ind.three.alt$code[list.ind.three.alt$g.bts.hts == "bts"]))
list.hts <- c(list.ps, ind.aa)
...
## Countries that need manutal treatment
...
list.orig <- list(list.ind, list.g, list.cs, list.ps)
list.orig.alt <- list(list.ind, list.g, list.bts, list.hts)
...
## ## Use the following function to manually check special cases
## ex.num <- 11
## cov.gopd.manual[[ex.num]]
## cov.gopd.manual[[ex.num]]$ind

## Manual check
## The key is where you can find more aggregated sectors that cover CS or PS.
## For example, even if you don't see D90T93, D94T96, and D97T98, you can do the price aggregation for CS if you see D90T99.
## For this manual check, getting BTS and HTS split is hard.

## 1) CHE: Merge 69 ~ 82, 90 ~ 99 -> 1995 ~ 2017. Get 2018 somewhere else 69 ~ 82, you cannot do.
...
gopd.che <- get.gopd.special.years(dat.che, list.re, list(list.ind.che, list.g, list.cs.che, list.ps.che), paste0("X", 1995:2017))
gopd.che[, "X2018"] <- NA
...
gopd.jpn <- get.gopd.special(dat.jpn, list.re, list(list.ind.jpn, list.g, list.cs.jpn, list.ps.jpn))
...
gopd.lva <- get.gopd.special(dat.lva, list.re, list(list.ind.lva, list.g, list.cs.lva, list.ps))
gopd.lva.gs <- get.gopd.special(dat.lva, list.re.gs, list(list.ind.lva, list.g, c(list.cs.lva, list.ps)))

## 6) PRT: except last year

## 7) SVK: 90 ~ 99. (except last two years)
list.cs.svk <- list.cs.che
list.ind.svk <- c(list.g, list.cs.svk, list.ps)

dat.svk <- get.data.special(dat.gopd, "SVK", list.ind.svk)

gopd.svk <- get.gopd.special.years(dat.svk, list.re, list(list.ind.svk, list.g, list.cs.svk, list.ps), paste0("X", 1995:2016))
...
gopd.usa <- pi.qi.agg.reclass(dat.usa, list.re, list(list.ind.usa, list.g, list.cs.usa, list.ps), years.x, "X2015", "PROD", "PRDK", "PRDP", "fisher")
...
country.gopd.final <- c(country.gopd.ind, "ITA", "PRT", "CHE", "JPN", "SVK", "LVA", "USA")
length(country.gopd.final)

gopd <- rbind(gopd.ind.all, gopd.che, gopd.jpn, gopd.svk, gopd.lva, gopd.usa)
gopd$var <- ifelse(gopd$var == "PROD", "go.v", ifelse(gopd$var == "PRDK", "go.q", ifelse(gopd$var == "PRDP", "go.p", "")))

## For G-BTS-HTS, 18 countries (ITA, PRT one year missing)
country.gopd.final.alt <- c(country.gopd.ind, "ITA", "PRT")

gopd.alt <- gopd.ind.all.alt
gopd.alt$var <- ifelse(gopd.alt$var == "PROD", "go.v", ifelse(gopd.alt$var == "PRDK", "go.q", ifelse(gopd.alt$var == "PRDP", "go.p", "")))

## G-S
country.gopd.final.gs <- country.gopd.final
...
avail.gopd.tab <- avail.to.table(check.pd.avail(gopd, "go.p", c("g", "cs", "ps")), c("g", "cs", "ps"))
avail.gopd.tab.alt <- avail.to.table(check.pd.avail(gopd.alt, "go.p", c("g", "bts", "hts")), c("g", "bts", "hts"))
...
## ex.num <- 16
## cov.vapd.manual[[ex.num]]
## cov.vapd.manual[[ex.num]]$ind
...
vapd.isl <- get.vapd.special(dat.isl, list.re, list(list.ind.isl, list.g, list.cs.isl, list.ps))
...
vapd.usa <- pi.qi.agg.reclass(dat.usa, list.re, list(list.ind.usa, list.g, list.cs.usa, list.ps), years.x, "X2015", "VALU", "VALK", "VALP", "fisher")
...
vapd.aus <- get.vapd.special(dat.aus, list.re, list(list.ind.aus, list.g, list.cs.aus, list.ps))
vapd.aus.gs <- get.vapd.special(dat.aus, list.re.gs, list(list.ind.aus, list.g, c(list.cs.aus, list.ps)))

## 8) CHE: (difference with documentation) G, H, I information missing in 2018. Other than last year, doable. it's the same with GOPD

dat.che <- get.data.special(dat.vapd, "CHE", list.ind.che)
vapd.che <- get.vapd.special(dat.che, list.re, list(list.ind.che, list.g, list.cs.che, list.ps.che))
vapd.che$X2018 <- NA
...
vapd.gbr <- get.vapd.special(dat.gbr, list.re, list(list.ind.gbr, list.g, list.cs.gbr, list.ps))
...
vapd.jpn <- get.vapd.special(dat.jpn, list.re, list(list.ind.jpn, list.g, list.cs.jpn, list.ps.jpn))
...
vapd.kor <- get.vapd.special(dat.kor, list.re, list(list.ind.kor, list.g, list.cs.kor, list.ps))
vapd.kor.gs <- get.vapd.special(dat.kor, list.re.gs, list(list.ind.kor, list.g, c(list.cs.kor, list.ps)))

## 14) LVA: 90 ~ 99, Same with GOPD.

dat.lva <- get.data.special(dat.vapd, "LVA", list.ind.lva)
vapd.lva <- get.vapd.special(dat.lva, list.re, list(list.ind.lva, list.g, list.cs.lva, list.ps))
...
vapd.nor <- get.vapd.special(dat.nor, list.re, list(list.ind.nor, list.g, list.cs.nor, list.ps))
...
country.vapd.final <- c(country.vapd.ind, "ISL", "USA", "AUS", "CHE", "GBR", "JPN", "KOR", "LVA", "NOR")
length(country.vapd.final)

vapd <- rbind(vapd.ind.all, vapd.isl, vapd.usa, vapd.aus, vapd.che, vapd.gbr, vapd.jpn, vapd.kor, vapd.lva, vapd.nor)
vapd$var <- ifelse(vapd$var == "VALU", "v", ifelse(vapd$var == "VALK", "q", ifelse(vapd$var == "VALP", "p", "")))

## 22 for G-BTS-HTS split.
country.vapd.final.alt <- country.vapd.ind
length(country.vapd.final.alt)

vapd.alt <- vapd.ind.all.alt
vapd.alt$var <- ifelse(vapd.alt$var == "VALU", "v", ifelse(vapd.alt$var == "VALK", "q", ifelse(vapd.alt$var == "VALP", "p", "")))

## 31 for G-S
country.vapd.final.gs <- country.vapd.final
...
avail.vapd.tab <- avail.to.table(check.pd.avail(vapd, "p", c("g", "cs", "ps")), c("g", "cs", "ps"))
avail.vapd.tab.alt <- avail.to.table(check.pd.avail(vapd.alt, "p", c("g", "bts", "hts")), c("g", "bts", "hts"))
...
list.re.wt <- c("g", "cs", "ps") # List of reclassified sector without total
...
writeLines(paste("Among", nrow(res.gopd.vapd), "country-sectors,", sum(res.gopd.vapd$cor.improve == "Y"), sum(res.gopd.vapd$rmse.improve == "Y"), sum(res.gopd.vapd$beta.improve == "Y"), "improved based on correlation, RMSE, and regression coefficients, respectively."), "../../doc/nums/imputed_gopd_vs_vapd.txt")
...
sum.gopd.vapd$ind <- factor(sum.gopd.vapd$ind, levels = c("g", "ps", "cs"))
...
ggplot(dat.country, aes(x = year, y = value, color = var)) +
    ...
    geom_line(aes(linetype = var), linewidth = 0.8) +
    geom_dl(aes(label = var), method = list(dl.trans(x = x - 0.2), "first.points", cex = 1)) +
    ylab("Price deflators (2015 = 1)") +
    plot.time.options(x.limits = c(1992.5, 2018))
dev.off()
...
## Get the value-added share for CS and PS and for BTS and HTS
...
list.re.valu <- c("45T47", "49T53", "55T56", "PS_58T98", "CS_58T98")
list.re.valu.dis <- list("45T47", c("49", "50", "51", "52", "53"), "55T56", c("58T60", "61", "62T63", "64T66", "69T75", "77T82"), c("68", "84", "85", "86T88", "90T93", "94T96", "97T98"))

## For the alternative industry classification (HTS and BTS)
...
list.re.valu.alt <- c("45T56", "HTS_58T98", "BTS_58T98")
list.re.valu.dis.alt <- list(c("45T47", "49", "50", "51", "52", "53", "55T56"), c("58T60", "61", "62T63", "64T66", "69T75", "77T82", "90T93"), c("68", "84", "85", "86T88", "94T96", "97T98"))

list.re.valu.alt.2 <- c("45T88", "90T93", "94T98")
list.re.valu.dis.alt.2 <- list(c("45T47", "49", "50", "51", "52", "53", "55T56", "58T60", "61", "62T63", "64T66", "68", "69T75", "77T82","84", "85", "86T88"), "90T93", c("94T96", "97T98")) 
...
setequal(unlist(list.re.valu.dis), unlist(list.re.valu.dis.alt.2))
length(unlist(list.re.valu.dis)) == length(unlist(list.re.valu.dis.alt))
length(unlist(list.re.valu.dis)) == length(unlist(list.re.valu.dis.alt.2))
...
icio.valu.alt.2 <- valu.agg(list.re.valu.alt.2, list.re.valu.dis.alt.2)
...
for (i in 1:length(icio.valu)){
    ...
    icio.valu.alt.2[[i]][, "year"] <- years.x[i]
}
...
icio.valu.alt.2 <- do.call("rbind", lapply(icio.valu.alt.2, function(dat){
    dat <- pivot_longer(dat, cols = -year, names_sep = "\\.", names_to = c("country", "ind"))
    dat <- pivot_wider(dat, names_from = "ind", values_from = "value")
...
icio.valu.I.CS.GTI <- pivot_wider(icio.valu[, c("country", "year", "I.CS.share.GTI")], names_from = "year", values_from = "I.CS.share.GTI")
icio.valu.H.CS.GH <- pivot_wider(icio.valu[, c("country", "year", "H.CS.share.GH.rev3")], names_from = "year", values_from = "H.CS.share.GH.rev3")
icio.valu.PS.JP <- pivot_wider(icio.valu[, c("country", "year", "PS.share.JP.rev3")], names_from = "year", values_from = "PS.share.JP.rev3")

## This we need for BTS-HTS division
icio.valu.R.HTS.RTU <- pivot_wider(icio.valu.alt.2[, c("country", "year", "R.HTS.share.RTU")], names_from = "year", values_from = "R.HTS.share.RTU")
icio.valu.HTS.JP <- pivot_wider(icio.valu.alt[, c("country", "year", "HTS.share.JP.rev3")], names_from = "year", values_from = "HTS.share.JP.rev3")
...
vapd.un.2 <- conversion.un(un.na, "v", "q", "p", c("total", "g", "cs", "ps"), list(c("_T"), c("A", "BTE"), c("I", "F", "L", "OTQ", "R", "STU"), c("G_H", "J", "K", "M_N")), setdiff(avail.country$country[avail.country$type == 2], "CHL"))

vapd.un.2.alt <- conversion.un(un.na, "v", "q", "p", c("total", "g", "bts", "hts"), list(c("_T"), c("A", "BTE"), c("F", "L", "OTQ", "STU"), c("G_H", "I", "J", "K", "M_N", "R")), setdiff(avail.country$country[avail.country$type == 2], "CHL"))
...
vapd.un.chl <- conversion.un(un.na, "v", "q", "p", c("total", "g", "cs", "ps"), list(c("_T"), c("A", "BTE"), c("I", "F", "L", "OTQ"), c("G_H", "J", "K", "M_N")), "CHL")

vapd.un.chl.alt <- conversion.un(un.na, "v", "q", "p", c("total", "g", "bts", "hts"), list(c("_T"), c("A", "BTE"), c("F", "L", "OTQ"), c("G_H", "I", "J", "K", "M_N")), "CHL")
...
vapd.un.3 <- conversion.un(un.na, "v", "q", "p", c("total", "g", "cs", "ps"), list(c("_T"), c("A", "BTE"), c("I", "F", "L", "R", "STU"), c("G_H", "K")), avail.country$country[avail.country$type == 3])

vapd.un.3.alt <- conversion.un(un.na, "v", "q", "p", c("total", "g", "bts", "hts"), list(c("_T"), c("A", "BTE"), c("F", "L", "STU"), c("G_H", "I", "K", "R")), avail.country$country[avail.country$type == 3])
...
vapd.un.4 <- conversion.un(un.na, "v", "q", "p", c("total", "g", "cs", "ps"), list(c("_T"), c("A", "BTE"), c("I", "F", "L", "OTQ"), c("G_H")), avail.country$country[avail.country$type == 4])

vapd.un.4.alt <- conversion.un(un.na, "v", "q", "p", c("total", "g", "bts", "hts"), list(c("_T"), c("A", "BTE"), c("F", "L", "OTQ"), c("G_H", "I")), avail.country$country[avail.country$type == 4])
...
vapd.un.7 <- conversion.un(un.na, "v", "q", "p", c("total", "g", "cs", "ps"), list(c("_T"), c("A", "BTE"), c("I", "F", "L", "OTQ", "R", "STU"), c("G_H", "J", "K")), avail.country$country[avail.country$type == 7])

vapd.un.7.alt <- conversion.un(un.na, "v", "q", "p", c("total", "g", "bts", "hts"), list(c("_T"), c("A", "BTE"), c("F", "L", "OTQ", "STU"), c("G_H", "I", "J", "K", "R")), avail.country$country[avail.country$type == 7])
...
avail.vapd.un.sdmx.tab <- avail.vapd.un.sdmx.tab.alt <- avail.vapd.un.sdmx.tab.gs <- avail.to.table(check.pd.avail(vapd.un, "p", c("g", "cs", "ps")), c("g", "cs", "ps"))
...
dat.un <- split.ind(dat.un, "G-H", "G", "H", icio.valu.H.CS.GH, with.p = FALSE)
dat.un <- split.ind(dat.un, "J-P", "J-P.CS", "J-P.PS", icio.valu.PS.JP, with.p = FALSE)

## Split J-P (Rev.3) into BTS and HTS part
dat.un.alt <- split.ind(dat.un.alt, "J-P", "J-P.BTS", "J-P.HTS", icio.valu.HTS.JP, with.p = FALSE)
...
vapd.un.ama <- conversion.un(dat.un, "v", "q", "p", c("total", "g", "cs", "ps"), list(c("TOTAL"), c("A-B", "C-E"), c("F", "H", "J-P.CS"), c("I", "G", "J-P.PS")), unique(dat.un$country))

vapd.un.ama.gs <- conversion.un(dat.un, "v", "q", "p", c("total", "g", "s"), list(c("TOTAL"), c("A-B", "C-E"), c("F", "H", "J-P.CS", "I", "G", "J-P.PS")), unique(dat.un$country))

vapd.un.ama.alt <- conversion.un(dat.un.alt, "v", "q", "p", c("total", "g", "bts", "hts"), list(c("TOTAL"), c("A-B", "C-E"), c("F", "J-P.BTS"), c("I", "G-H", "J-P.HTS")), unique(dat.un.alt$country))

avail.vapd.un.ama.tab <- avail.vapd.un.ama.tab.alt <- avail.vapd.un.ama.tab.gs <- avail.to.table(check.pd.avail(vapd.un.ama, "p", c("g", "cs", "ps")), c("g", "cs", "ps"))
...
## Derive g, cs, ps deflators for 1995 ~ 2018.
dat.twn[ paste0("X", c(1981:1994, 2019:2020))] <- NULL

vapd.twn <- pi.qi.agg.reclass(dat.twn, c("total", "g", "cs", "ps"), list(toupper(letters[1:19]), toupper(letters[1:5]), c("F", "I", "L", "O", "P", "Q", "R", "S"), c("G", "H", "J", "K", "M", "N")), paste0("X", 1995:2018), "X2015", "v", "q", "p", "laspeyres")
...
vapd.twn.alt <- pi.qi.agg.reclass(dat.twn, c("total", "g", "bts", "hts"), list(toupper(letters[1:19]), toupper(letters[1:5]), c("F", "L", "O", "P", "Q", "S"), c("I", "R", "G", "H", "J", "K", "M", "N")), paste0("X", 1995:2018), "X2015", "v", "q", "p", "laspeyres")
...
res.vapd.detail$ind <- factor(res.vapd.detail$ind, levels = c("G", "PS", "CS"))
...
## "vapd" contains no empty entries for price deflators except CHE.
...
vapd.final <- vapd[vapd$var == "p", ]
vapd.final.alt <- vapd.alt[vapd.alt$var == "p", ]
vapd.final.gs <- vapd.gs[vapd.gs$var == "p", ]

## 32 countries (G-CS-PS and G-S) and 23 countries (G-BTS-HTS)
nrow(vapd.final)/4
nrow(vapd.final.gs)/3
nrow(vapd.final.alt)/4

## CHE special treatment
## Add last year info from UN MAIN
add.che.last <- function(dat.orig, dat.add){
    dat.che <- dat.orig[dat.orig$country == "CHE", ]
    dat.orig <- dat.orig[dat.orig$country != "CHE", ]

    dat.che.un <- dat.add[dat.add$var == "p" & dat.add$country == "CHE", c("X2017", "X2018", "ind", "country")]
    dat.che.un$X2018.2017 <- dat.che.un$X2018/dat.che.un$X2017
    dat.che.un[, c("X2017", "X2018")] <- NULL
    
    dat.che <- merge(dat.che, dat.che.un, by = c("country", "ind"))
    dat.che$X2018 <- dat.che$X2017*dat.che$X2018.2017
    dat.che$X2018.2017 <- NULL
    
    dat.orig <- rbind(dat.orig, dat.che)
    return(dat.orig)
}

vapd.final <- add.che.last(vapd.final, vapd.un)
vapd.final.gs <- add.che.last(vapd.final.gs, vapd.un.gs)
...
update.main.ama <- function(list.country, list.re, vapd.un, vapd.un.ama){

    vapd.un.updated <- subset(vapd.un, country %in% list.country)

    for (ex.country in list.country){
        for (i.ind in list.re){
            ## Not every entry is provided in "vapd.un",
            if (!(prod(!is.na(vapd.un[vapd.un$country == ex.country & vapd.un$ind == i.ind & vapd.un$var == "p", years.x])) == 1)){
                na.years.ind <- as.numeric(is.na(vapd.un[vapd.un$country == ex.country & vapd.un$ind == i.ind & vapd.un$var == "p", years.x]))
                ...
                if (length(ind.first) == 1){ # length = 0 means that there was no missing entry in the first part of the time period.
                    ...
                    vapd.un.updated[vapd.un.updated$country == ex.country & vapd.un.updated$ind == i.ind & vapd.un.updated$var == "p", na.years.first] <- vapd.un.ama[vapd.un.ama$country == ex.country & vapd.un.ama$ind == i.ind & vapd.un.ama$var == "p", na.years.first]/vapd.un.ama[vapd.un.ama$country == ex.country & vapd.un.ama$ind == i.ind & vapd.un.ama$var == "p", years.x[ind.first]]*vapd.un[vapd.un$country == ex.country & vapd.un$ind == i.ind & vapd.un$var == "p", years.x[ind.first]] # Price sequences from UN AMA/ UN AMA first entry year * UN NA_MAIN first entry year
            }
                ...
                if (length(ind.last) == 1){
                    ...
                    vapd.un.updated[vapd.un.updated$country == ex.country & vapd.un.updated$ind == i.ind & vapd.un.updated$var == "p", na.years.last] <- 
                        vapd.un.ama[vapd.un.ama$country == ex.country & vapd.un.ama$ind == i.ind & vapd.un.ama$var == "p", na.years.last]/vapd.un.ama[vapd.un.ama$country == ex.country & vapd.un.ama$ind == i.ind & vapd.un.ama$var == "p", years.x[ind.last]]*vapd.un[vapd.un$country == ex.country & vapd.un$ind == i.ind & vapd.un$var == "p", years.x[ind.last]]
                }
            }
        }
    }
    return(vapd.un.updated)
}

vapd.un.updated <- update.main.ama(country.vapd.un.only, list.re, vapd.un, vapd.un.ama)

vapd.un.updated.gs <- update.main.ama(country.vapd.un.only.gs, list.re.gs, vapd.un.gs, vapd.un.ama.gs)

vapd.un.updated.alt <- update.main.ama(country.vapd.un.only.alt, list.re.alt, vapd.un.alt, vapd.un.ama.alt)

## Sanity check for one country
ex.country <- "CAN"

subset(vapd.un.updated, country == ex.country & var == "p" & ind == "cs")
subset(vapd.un, country == ex.country & var == "p" & ind == "cs")
subset(vapd.un.ama, country == ex.country & var == "p" & ind == "cs")

subset(vapd.un.updated.alt, country == ex.country & var == "p" & ind == "bts")
subset(vapd.un.alt, country == ex.country & var == "p" & ind == "bts")
subset(vapd.un.ama.alt, country == ex.country & var == "p" & ind == "bts")

## Add it to the data (countries in STAN + UN NA_MAIN)
vapd.final <- rbind(vapd.final, subset(vapd.un.updated, var == "p"))
nrow(vapd.final)/4 # 32 + 19 countries

vapd.final.gs <- rbind(vapd.final.gs, subset(vapd.un.updated.gs, var == "p"))
nrow(vapd.final.gs)/3 # 32 + 19 countries

vapd.final.alt <- rbind(vapd.final.alt, subset(vapd.un.updated.alt, var == "p"))
...
vapd.final <- rbind(vapd.final, subset(vapd.un.ama, country %in% country.vapd.unama.only & var == "p"))

nrow(vapd.final) == 66*4

vapd.final.gs <- rbind(vapd.final.gs, subset(vapd.un.ama.gs, country %in% country.vapd.unama.only & var == "p"))

nrow(vapd.final.gs) == 66*3

vapd.final.alt <- rbind(vapd.final.alt, subset(vapd.un.ama.alt, country %in% country.vapd.unama.only & var == "p"))
...
list.re.wt <- c("g", "cs", "ps") # List of reclassified sector without total
list.re.wt.gs <- c("g", "s")
list.re.wt.alt <- c("g", "bts", "hts")

list.reclass <- c("g", "cs", "ps")
...
check.missing <- function(country.gopd.final, gopd) country.gopd.final[!unlist(lapply(country.gopd.final, function(ex.country) (prod(!is.na(subset(gopd, country == ex.country & var == "go.p")[, years.x])) == 1)))]

check.missing(country.gopd.final, gopd)
check.missing(country.gopd.final.gs, gopd.gs)
check.missing(country.gopd.final.alt, gopd.alt)

## Missing info: ITA, PRT, CHE, SVK for g-ps-cs and g-s & ITA, PRT for g-hts-bts

## Updating years for country
update.gopd <- function(gopd, gopd.con, ex.country, miss.years, list.re.wt){
    x.years <- paste0("X", c(miss.years[1]-1, miss.years))
    ## Derive prices for missing years using constructed GOPD
    country.con <- subset(gopd.con, var == "go.p.con" & country == ex.country)[, c("ind", x.years)]
    colnames(country.con) <- c("ind", paste0(x.years, ".con"))
    country.orig <- subset(gopd, var == "go.p" & country == ex.country)[, c("ind", x.years)]
    country.convert <- merge(country.orig, country.con, by = "ind", all.x = TRUE)

    for (i.year in x.years[2:length(x.years)]){
        country.convert[, paste0(i.year, ".new")] <- country.convert[, paste0(i.year, ".con")]/country.convert[, paste0(x.years[1], ".con")]*country.convert[, x.years[1]]
        country.convert[is.na(country.convert[, i.year]), i.year] <- country.convert[is.na(country.convert[, i.year]), paste0(i.year, ".new")]
    }
    ## Update the GOPD
    for (i.year in x.years[2:length(x.years)]){
        for (i.ind in list.re.wt){
            gopd[gopd$var == "go.p" & gopd$ind == i.ind & gopd$country == ex.country, i.year] <- country.convert[country.convert$ind == i.ind, i.year]
        }
    }
    return(gopd)    
}

## Update missing infos
gopd <- update.gopd(gopd, gopd.con, "CHE", 2018, list.re.wt)
gopd <- update.gopd(gopd, gopd.con, "ITA", 2018, list.re.wt)
gopd <- update.gopd(gopd, gopd.con, "PRT", 2018, list.re.wt)
gopd <- update.gopd(gopd, gopd.con, "SVK", 2017:2018, list.re.wt)

gopd.gs <- update.gopd(gopd.gs, gopd.con.gs, "CHE", 2018, list.re.wt.gs)
gopd.gs <- update.gopd(gopd.gs, gopd.con.gs, "ITA", 2018, list.re.wt.gs)
gopd.gs <- update.gopd(gopd.gs, gopd.con.gs, "PRT", 2018, list.re.wt.gs)
gopd.gs <- update.gopd(gopd.gs, gopd.con.gs, "SVK", 2017:2018, list.re.wt.gs)

gopd.alt <- update.gopd(gopd.alt, gopd.con.alt, "ITA", 2018, list.re.wt.alt)
gopd.alt <- update.gopd(gopd.alt, gopd.con.alt, "PRT", 2018, list.re.wt.alt)

## Subset only price variables
gopd.final <- subset(gopd, var == "go.p" & ind %in% list.re.wt)
gopd.final.gs <- subset(gopd.gs, var == "go.p" & ind %in% list.re.wt.gs)
gopd.final.alt <- subset(gopd.alt, var == "go.p" & ind %in% list.re.wt.alt)

## Other countries from constructed gopd
add.con <- function(gopd.final, gopd.con){
    gopd.final <- rbind(gopd.final, subset(gopd.con, country %in% setdiff(country.final, unique(gopd.final$country))))
    return(gopd.final)
}

gopd.final <- add.con(gopd.final, gopd.con)
gopd.final.gs <- add.con(gopd.final.gs, gopd.con.gs)
gopd.final.alt <- add.con(gopd.final.alt, gopd.con.alt)
...
gopd.final[gopd.final$p.usd < 0, ] ## Brunei, negative GOPD for g. Possibly because of oil prices fluctuation. For BRN, use value-added prices
gopd.final.gs[gopd.final.gs$p.usd < 0, ]
gopd.final.alt[gopd.final.alt$p.usd < 0, ]

substitute.brn <- function(gopd.final, vapd.final, list.re.wt){

    gopd.final <- subset(gopd.final, country != "BRN")

    gopd.final[gopd.final$country == "BRN", c("p.lcu", "p.usd")] <- NA

    vapd.brn <- pivot_longer(vapd.final[vapd.final$country == "BRN", ], cols = all_of(years.x), names_to = "year")
    prod(vapd.brn$var == "p")
    vapd.brn$var <- NULL
    vapd.brn <- subset(vapd.brn, ind %in% list.re.wt)
    vapd.brn <- clean.colnames(vapd.brn, "value", "p.lcu")
    vapd.brn <- merge(vapd.brn, ner, by = c("country", "year"), all.x = TRUE)
    vapd.brn$p.usd <- vapd.brn$p.lcu/vapd.brn$ner.2015

    gopd.final <- rbind(gopd.final, vapd.brn)
    return(gopd.final)
}

gopd.final <- substitute.brn(gopd.final, vapd.final, list.re.wt)
gopd.final.gs <- substitute.brn(gopd.final.gs, vapd.final.gs, list.re.wt.gs)
gopd.final.alt <- substitute.brn(gopd.final.alt, vapd.final.alt, list.re.wt.alt)

gopd.final[gopd.final$p.usd < 0, ]
gopd.final.gs[gopd.final.gs$p.usd < 0, ]
gopd.final.alt[gopd.final.alt$p.usd < 0, ]
